...
Now that we have our data, we can use it to plot a picture of the hole in the ozone layer. Note that the "hole" in the ozone layer is the light regions around the south pole. The circular white area centered on the south pole shows where the satellite did not take measurements, and the small gray region shows the missing values. Let's plot this using an orthographic projection centered over the South Pole.

Note: remove `eval = FALSE` from the chunk header below, and then delete this line.

```{r, eval = FALSE, echo = FALSE, fig.width = 6, fig.height = 6}
ortho <- "+proj=ortho +lat_0=-90 +lon_0=0"
world <- <--obtain world data from rnaturalearth--->
nimbus_sf <- st_as_sf(nimbus, coords = c("longitude", "latitude"), crs = 4326)

ggplot() +
  <---nimbus layer with aesthetic mappings--->) +
...
```{r eval = FALSE}
write_csv(nimbus, path = here("09_data-import/data/nimbus-clean.csv"))
```
