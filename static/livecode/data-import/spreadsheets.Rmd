...
```{r}
stickers_path <- here("08_data-import/spreadsheets", "PosidFazioCordes_StickerRich_Excel-Dataverse.xlsx")
```
...
That second row of data is wonky, and we can just get rid of it. There's a nice `dplyr` function called `slice()` for choosing rows based on their position in the table. Here's a demostration:
...
We can fix that with the `dplyr` function `rename()`. Here's a basic example of how you use this function:

```{r, eval = FALSE}
rename(your_data, new_col_name = old_col_name)
```

Note the normal variable names can be bare (unquoted), but for our name that is full of spaces and punctuation we'll need to use quotes around the old name. 

Finally, we want to make cleaner names. The R package `janitor` provides a handy function `clean_names()` for making nice names using a variety of rules. By default, it makes names using [snake_case](https://en.wikipedia.org/wiki/Snake_case), but you can find all the other options in the [documentation](http://sfirke.github.io/janitor/reference/clean_names.html), including such delightful naming rules as "big_camel" (NamesLikeThis) and "screaming_sname" (NAMES_LIKE_THIS).
...
```{r}
artifacts_path <- here("08_data-import/spreadsheets", "messy_ktc_data.xlsx")
```
