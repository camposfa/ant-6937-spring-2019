...
- The `ymd()` function from the `lubridate` package understands that these are actually dates.
...
Or if we want to get fancy, we can split the series up into a positive and a negative set so we can use different fill colors for the El Niño (positive, warm) and La Niña (negative, cool) phases. Crucially, the positive and negative sets cannot just be plotted well when they're created using `filter()` (you can try it if you want).

- To make the positive set, use `if_else()` in `mutate()` to modify the `oni` variable so that if the value is less than 0, it gets assigned the value zero.
